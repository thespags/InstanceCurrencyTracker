...
local function createSlider(parent, name, x, y, key, min, max)
    ...
    frame:SetValue(ICT.db.options[key])
...
function MainOptions:init(parent)
    ...
    -- Maybe confusing for new users if filter is set to max, so make it 1 and let them change it.
    ICT.db.options.minimumLevel = ICT.db.options.minimumLevel or 1
