...
lib.dummyFrame = CreateFrame("Frame")
lib.barFrameMT = {__index = lib.dummyFrame}
lib.barPrototype = setmetatable({}, lib.barFrameMT)
lib.barPrototype_mt = {__index = lib.barPrototype}
...
function barPrototype:SetValue(value, maxValue)
   ...
   if self.maxValue > 0 then
      amt = math.min(1, value / self.maxValue)
   end
...
function barPrototype:SetTextureValue(amt, dist)
   ...
   amt = math.max(amt, 0.000001) 
   t:SetValue(amt * math.max(dist, 0.000001))
   if o == 1 then
